...
import { pushApi } from "@/features/push/api/pushApi";
  ...
  async enable() {
    ...
    try {
      ...
      if (permission !== "granted") {
        ...
        if (permission === "denied") {
          set({
            feedback: "Разрешение на уведомления отклонено в браузере.",
          });
        }
      ...
      set({
        ...
        capabilityState: "enabled",
        lastKnownEndpoint: subscription.endpoint,
        feedback: "Push-уведомления включены.",
      });
      return true;
    } catch (error) {
      set({
        isLoading: false,
        feedback: mapPushErrorMessage(
          error,
  ...
  async disable() {
    ...
    try {
      ...
      set({
        ...
        lastKnownEndpoint: null,
        feedback: "Push-уведомления отключены.",
      });
      return true;
    } catch (error) {
      set({
